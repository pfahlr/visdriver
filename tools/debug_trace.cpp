...
// Helper to resolve the genuine system export before our hooks patch the IAT
// thunks. Falling back to the import keeps us functional if the module cannot
// be located (e.g., on older Windows variants) but still avoids recursion when
// resolution succeeds.
template <typename T>
T ResolveOriginalFunction(const wchar_t *module_name, const char *function_name,
                          T fallback) {
...
decltype(&CreateDIBSection) g_orig_CreateDIBSection =
    ResolveOriginalFunction(L"gdi32.dll", "CreateDIBSection", CreateDIBSection);
...
decltype(&ChoosePixelFormat) g_orig_ChoosePixelFormat =
    ResolveOriginalFunction(L"gdi32.dll", "ChoosePixelFormat", ChoosePixelFormat);
decltype(&SetPixelFormat) g_orig_SetPixelFormat =
    ResolveOriginalFunction(L"gdi32.dll", "SetPixelFormat", SetPixelFormat);
decltype(&wglCreateContext) g_orig_wglCreateContext =
    ResolveOriginalFunction(L"opengl32.dll", "wglCreateContext", wglCreateContext);
decltype(&wglMakeCurrent) g_orig_wglMakeCurrent =
    ResolveOriginalFunction(L"opengl32.dll", "wglMakeCurrent", wglMakeCurrent);
decltype(&wglDeleteContext) g_orig_wglDeleteContext =
    ResolveOriginalFunction(L"opengl32.dll", "wglDeleteContext", wglDeleteContext);
...
decltype(&DescribePixelFormat) g_orig_DescribePixelFormat =
    ResolveOriginalFunction(L"gdi32.dll", "DescribePixelFormat",
                            DescribePixelFormat);
